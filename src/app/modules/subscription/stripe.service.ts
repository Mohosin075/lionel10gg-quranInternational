...
class StripeService {
  ...
  constructWebhookEvent(
    ...
    try {
      const secret = webhookSecret || config.stripe.paymentWebhookSecret || config.stripe.subscriptionWebhookSecret
      if (!secret) {
  ...
  async archivePrice(priceId: string): Promise<any> {
    try {
  ...
  async retryInvoicePayment(invoiceId: string): Promise<any> {
    try {
