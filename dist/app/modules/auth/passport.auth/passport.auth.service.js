...
const handleGoogleLogin = async (payload) => {
    ...
    if (isUserExist) {
        ...
        return (0, common_1.authResponse)(http_status_codes_1.StatusCodes.OK, `Welcome ${isUserExist.name} to Quran International.`, isUserExist.role, tokens.accessToken, tokens.refreshToken);
    }
    ...
    try {
        ...
        session.endSession();
        return (0, common_1.authResponse)(http_status_codes_1.StatusCodes.OK, `Welcome ${user[0].name} to Quran International.`, user[0].role, tokens.accessToken, tokens.refreshToken);
    }
    catch (error) {
