...
const customLogin = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const result =
    await custom_auth_service_1.CustomAuthServices.customLogin(loginData)
...
const adminLogin = (0, catchAsync_1.default)(async (req, res) => {
  const { ...loginData } = req.body
  const result =
    await custom_auth_service_1.CustomAuthServices.adminLogin(loginData)
  const { status, message, accessToken, refreshToken, role } = result
  res.cookie('refreshToken', refreshToken, {
    secure: process.env.NODE_ENV === 'production',
    httpOnly: true,
...
const forgetPassword = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const result = await custom_auth_service_1.CustomAuthServices.forgetPassword(
...
const resetPassword = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const result = await custom_auth_service_1.CustomAuthServices.resetPassword(
    token,
    resetData,
  )
  ;(0, sendResponse_1.default)(res, {
...
const verifyAccount = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const result = await custom_auth_service_1.CustomAuthServices.verifyAccount(
    email,
    oneTimeCode,
  )
...
const getRefreshToken = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const result =
    await custom_auth_service_1.CustomAuthServices.getRefreshToken(refreshToken)
  ;(0, sendResponse_1.default)(res, {
    statusCode: http_status_codes_1.StatusCodes.OK,
    success: true,
...
const changePassword = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const result = await custom_auth_service_1.CustomAuthServices.changePassword(
    req.user,
    currentPassword,
    newPassword,
  )
...
const createUser = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const result =
    await custom_auth_service_1.CustomAuthServices.createUser(userData)
...
const deleteAccount = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const result = await custom_auth_service_1.CustomAuthServices.deleteAccount(
    user,
    password,
  )
  ;(0, sendResponse_1.default)(res, {
...
const socialLogin = (0, catchAsync_1.default)(async (req, res) => {
  ...
  const result = await custom_auth_service_1.CustomAuthServices.socialLogin(
    appId,
    deviceToken,
  )
  const { status, message, accessToken, refreshToken, role } = result
