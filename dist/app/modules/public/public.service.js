...
const createPublic = async payload => {
  ...
  if (isExist) {
    await public_model_1.Public.findByIdAndUpdate(
      isExist._id,
      {
        $set: {
...
const updateFaq = async (id, payload) => {
  const result = await public_model_1.Faq.findByIdAndUpdate(
    id,
    { $set: payload },
    {
      new: true,
...
const updatePublic = async (id, payload) => {
  ...
  const result = await public_model_1.Public.findByIdAndUpdate(
    id,
    { $set: updateData },
    { new: true },
  )
